...
%DIF ADD main.tex               Wed Jul 12 21:26:24 2017
...
%DIF -------
\title{Challenging a bioinformatic tool's ability to detect microbial contaminants using \textit{in silico} whole genome sequencing data.} %DIF > 
%DIF -------
...
For the individual genomes the false positive contaminants depended on the genus with \textit{Staphylococcus}, \textit{Escherichia}, and \textit{Shigella} having the highest proportion of false positives.  %DIF > 
For nearly all binary mixtures the contaminant was detected in the \textit{in-silico} datasets at the equivalent of 1 in 1,000 cells, with the exceptions of \textit{F. tularensis}, which was not detected in any of the simulated contaminant mixtures, and \textit{Y. pestis}, which was only detected at the equivalent of 1 in 10 cells.  %DIF > 
Once a WGS method for detecting contaminants is characterized, it can be applied to evaluate microbial material purity, in efforts to ensure that contaminants are characterized and minimized in microbial materials used to validate pathogen detection assays, generate genome assemblies for database submission, and benchmark sequencing methods. %DIF > 
%DIF -------
\end{abstract}
...
High }\DIFaddend sensitivity detection methods including polymerase chain reaction (PCR) and next generation sequencing (NGS) can detect organismal contaminants previously undetectable by traditional \DIFdelbegin \DIFdel{microbiology methodssuch as culturing, biochemical tests, and microscopy. 
Characterizing and reducing the level of these contaminants }\DIFdelend \DIFaddbegin \DIFadd{microbiological methods. 
Characterizing these contaminants in order to focus efforts on reducing their level }\DIFaddend is critical to ensuring \DIFdelbegin \DIFdel{high quality }\DIFdelend \DIFaddbegin \DIFadd{high-quality }\DIFaddend microbial materials are used to populate sequence databases \citep{parks2015checkm}, for mock communities \DIFdelbegin \DIFdel{used }\DIFdelend to validate metagenomic methods \citep{bokulich2016mockrobiota}, to validate biodetection assays  \citep{Ieven2013,International2011}, and for basic research using model systems \citep{Shrestha2013}. 
...
\DIFdelbegin \DIFdel{In addition to organismal contaminants in the material itself, }\DIFdelend \DIFaddbegin \DIFadd{Contaminants in microbial materials, as found in non-axenic cellular materials or genomic materials with foreign DNA, have been addressed when processing the sequencing data but not for general material characterization \mbox{%DIFAUXCMD
\citep{Shrestha2013,tennessen2015prodege}}%DIFAUXCMD
.
}\DIFaddend PCR and NGS can also detect reagent impurities\DIFdelbegin \DIFdel{, highlighting the need to differentiate material and reagent contaminants. 
Issues related to reagent contaminants are well documented and addressed with }\DIFdelend \DIFaddbegin \DIFadd{. 
Reagent contaminants can be addressed by producing }\DIFaddend negative controls \citep{jervis2015deriving}, improved methods for removing contaminants \citep{woyke2011decontamination,motley2014improved}, and post-processing of sequence data \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{mukherjee2015large}}%DIFAUXCMD
. 
However, contaminants in microbial materials, as found in non-axenic cellular materials or genomic materials with foreign DNA, has only been addressed when processing the sequencing data and not material characterization \mbox{%DIFAUXCMD
\citep{Shrestha2013,tennessen2015prodege}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{in effort to distinguish these impurities from true organismal contaminant in the microbial material \mbox{%DIFAUXCMD
\citep{mukherjee2015large}}%DIFAUXCMD
}\DIFaddend . 
...
Culture }\DIFdelend \DIFaddbegin \DIFadd{typically fail to meet all the requirements to characterize microbial materials for routine applications. 
...
In \DIFaddbegin \DIFadd{order to confidently use metagenomics to detect contaminants in microbial materials, one must first understand its limitations in doing so. 
We have developed a metagenomics-based approach to evaluate contaminant detection capabilities. 
In }\DIFaddend this work, we present results from an \textit{in-silico} study demonstrating \DIFdelbegin \DIFdel{the use of whole genome sequence data combined with a }\DIFdelend \DIFaddbegin \DIFadd{our approach using an existing }\DIFaddend taxonomic assignment algorithm for detecting contaminant DNA in \DIFdelbegin \DIFdel{microbial materials.
...
Sequence data was simulated for 406 strains, from \DIFdelbegin \DIFdel{9 }\DIFdelend \DIFaddbegin \DIFadd{ten }\DIFaddend genera (Table \ref{tab:single_org}, Supplemental Table 1).
...
Due to the large number of \DIFdelbegin \textit{\DIFdel{Bacillus}}%DIFAUXCMD
\DIFdel{, }\DIFdelend \textit{Escherichia} \DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend \textit{\DIFdelbegin \DIFdel{Salmonella}\DIFdelend \DIFaddbegin \DIFadd{Staphylococcus}\DIFaddend } genomes, genomes from these genera were limited to the species \DIFdelbegin \textit{\DIFdel{Bacillus cereus}}%DIFAUXCMD
...
\DIFaddbegin \DIFadd{We note that after the genomes were selected one of the }\textit{\DIFadd{Staphylocuccus aureus}} \DIFadd{genomes was renamed }\textit{\DIFadd{S. argenteus}}\DIFadd{, Genbank taxid 985002 \mbox{%DIFAUXCMD
...
The }\textit{\DIFadd{Salmonella}} \DIFadd{and }\textit{\DIFadd{E. coli}} \DIFadd{had the highest percent of their genomes aligned to each other at 36 \% and 30 \%, respectively. 
}\DIFaddend 
...
\DIFaddbegin \DIFadd{Another potential reason is that genomes from both }\textit{\DIFadd{Y. pestis}} \DIFadd{and }\textit{\DIFadd{F. tularensis}} \DIFadd{have high insertion sequence content and readily undergo intragenomic recombination \mbox{%DIFAUXCMD
\citep{larsson2005complete,chain2004insights}}%DIFAUXCMD
. 
...
}\DIFaddend Additionally, the \textit{F. tularensis} dataset is much smaller relative to the other genera, less than 90,000 reads. 
...
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}
\includegraphics[width=\maxwidth]{figure/contam_fig-1} \caption[The relationship between the proportion of reads matching the contaminant species and the proportion of simulated contaminant reads]{The relationship between the proportion of reads matching the contaminant species and the proportion of simulated contaminant reads. Plots are \DIFdelbeginFL \DIFdelFL{split by }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{faceted on }\DIFaddendFL the \DIFaddbeginFL \DIFaddFL{x-axis by }\DIFaddendFL material species \DIFdelbeginFL \DIFdelFL{with line }\DIFdelendFL and \DIFdelbeginFL \DIFdelFL{point }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{on the y-axis by contaminant species. Point }\DIFaddendFL color \DIFdelbeginFL \DIFdelFL{indicating }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{indicates }\DIFaddendFL contaminant species \DIFaddbeginFL \DIFaddFL{and line color indicates material species}\DIFaddendFL . Dashed line indicates the expected 1:1 correlation between the proportion of reads matching the expected contaminant and the proportion of reads simulated from the contaminant. The contaminant proportion was underestimated for points below the dashed line and overestimated for points above the dashed line.}\label{fig:contam_fig}
\end{figure}
...
\DIFaddbegin \DIFadd{The proportion of reads from }\textit{\DIFadd{E. coli}} \DIFadd{and }\textit{\DIFadd{S. aureus}} \DIFadd{in the simulated contaminant datasets is consistently overestimated by PathoScope (Fig. \ref{fig:contam_fig}). 
...
%DIFDELCMD < %%%
\DIFdel{The potential for using whole genome sequence data and taxonomic sequence classification algorithms to detect contaminant DNA in microbial materials was evaluated.
The method requires no }\textit{\DIFdel{a priori}} %DIFAUXCMD
\DIFdel{information about the contaminant and can identify common as well as unexpected contaminants.
...
Overall the method was able to identify contaminant proportionsat $10^{-3}$ for most contaminant-material combinations.
...
\DIFaddbegin \DIFadd{We have developed an }\textit{\DIFadd{in-silico}} \DIFadd{approach to evaluate the ability of an existing taxonomic sequence classification algorithm to detect contaminant DNA in whole genome sequence datasets from microbial materials. 
The use of }\textit{\DIFadd{in-silico}} \DIFadd{data allows for a “known” contaminant taxonomy and concentration to challenge the algorithm. 
...
\DIFaddend There are three basic steps to using this method to detect contaminants in a microbial material. 
...
\DIFaddend The reference database and taxonomic assignment algorithm are likely to impact the number and types of false positives. 
...
.
Other methods such as MicrobeGPS and DUDes are alternative similarity based taxonomic classification methods developed to better handle organisms not in a reference database and are also suitable alternatives to PathoScope \mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{Removing sequences from the database }\DIFdelend \DIFaddbegin \DIFadd{Users can generate application specific databases by preparing a custom database without sequences }\DIFaddend for irrelevant contaminants, such as phage, plasmids, vectors, \DIFdelbegin \DIFdel{and }\DIFdelend multicellular eukaryotes, \DIFdelbegin \DIFdel{would }\DIFdelend \DIFaddbegin \DIFadd{and genes known to undergo horizontal gene transfer in order to }\DIFaddend reduce the proportion of false positives. 
By excluding irrelevant contaminants \DIFdelbegin \DIFdel{from the database}\DIFdelend \DIFaddbegin \DIFadd{and genes involved in horizontal gene transfer}\DIFaddend , sequencing reads aligning to \DIFdelbegin \DIFdel{these irrelevant }\DIFdelend \DIFaddbegin \DIFadd{the omitted }\DIFaddend sequences would no longer result in false positive contaminants.
...
For example, vector sequences from contaminants in sequencing reagents, if excluded from the database\DIFaddbegin \DIFadd{, }\DIFaddend may be incorrectly classified as an organismal contaminant.
...
\DIFdelbegin \DIFdel{The Department of Homeland Security (DHS) Science and Technology Directorate supported this work under the Interagency Agreement HSHQPM-15-T-00019 with the NIST}\DIFdelend \DIFaddbegin \DIFadd{We also thank Mihai Pop, Todd Treangen, Scott Jackson, and Jason Kralj for feedback on the manuscript. 
Additionally, we appreciate the comments and suggestions of the Academic Editor A. Murat Eren and the peer reviewers, which greatly improved the manuscript}\DIFaddend .
