...
\title{Using \textit{in-silico} whole genome sequencing  datasets to challenge the ability of an existing bioinformatic tool to detect contaminants in microbial materials.}
...
Whole genome sequencing (WGS) is a promising approach for detecting contaminants due to its sensitivity and lack of need for \textit{a priori} assumptions about the contaminant. Prior to applying WGS, we must first understand its limitations for detecting contaminants and potential for false positives. 
...
Simulated WGS datasets from ten genera as individuals and binary mixtures of eight organisms at varying ratios were analyzed to evaluate the role of contaminant concentration and taxonomy on detection. 
...
Additionally, PathoScope was successfully used in our pilot study (https://doi.org/10.6084/m9.figshare.1200090.v1) and as part of a pipeline we developed to characterize the NIST microbial genomic DNA reference material \citep{olson2016pepr}. 
...
Average nucleotide identity for all pair-wise comparisons was calculated using MUMMER3 and the \texttt{ani_pairs.R} script in the project github repository \citep{kurtz2004versatile}.
...
Our analysis included taxonomic classification results for simulated sequencing data  from \Sexpr{length(spec_prop)} genomes, representing 10 different genera (Table \ref{tab:single_org}, Fig. \ref{fig:single_org_tree}, Supplemental Table 1). 
The 10 genera were from the Gamma proteobacteria Class and Firmicutes phylum with 1 to 21 species representing a range of genomic similarity within the genera (Fig. \ref{baseline_ani}). 
For the genomes included in the study \textit{Escherichia}, \textit{Shigella}, \textit{Salmonella}, and \texit{Staphylococcus} tended to have higher within genus similarity whereas \textit{Clostridium} and \textit{Pseudomonas} had lower within genus similarity. 
...
Sequences can be genomically indistinguiable due to phylogenetic relatedness of the organisms as well as the transfer of sequences such as plasmids and genes involved in horizontal gene transfer events \citep{shintani2015genomics,polz2013horizontal}.
...
The representive genomes had low pairwise similarity based on the average nucleotide identity analysis (Fig. \ref{fig:contam_ani}) with average identity between 82 and 86\% with less than 1\% of the genomes algined for only 4 of the 28 organism pairs. 
...
Similarity between the material and contaminant genomes did not appear to impact the minimum contaminant proportion detected. 
However, this is likely due to the overall low level of similarity between the representative genomes (Fig. \ref{fig:contam_ani}). 
...
Users can generate application specific databases by removing sequences from the database for irrelevant contaminants, such as phage, plasmids, vectors, multicellular eukaryotes, and genes known to undergo horizontal gene transfer eould reduce the proportion of false positives. 
...
Similarly, the developers of the taxonomic classification algorithm Centrifuge provide multiple databases; only Prokaryotes; Prokaryotes and viruses; Prokaryotes, viruses, and human; as well as NCBI nucleotide non-redundant sequences. 
...
In summary, we have provided a straight-forward \textit{in-silico} approach using existing datasets to to challenge and evaluate the use of WGS for contaminant detection. 
