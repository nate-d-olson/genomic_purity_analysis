...
<<phage_prop, echo=FALSE, message=FALSE, fig.cap="Estimated total proportion of phage in the simulated single genome datasets by genera. Final Guess values reported by PathoScope used to calculate estimated total proportions. No phage were reported by PathoScope in any of the \\textit{Francisella} genomes.">>=
ggplot(phage_total) +
...
As the individual datasets were simulated at 20X coverage, \textless 300,000 reads were simulated for each dataset, and on average \textless 3 reads were spiked into the material datasets for simulated contaminant proportions less than $10^{-4}$ (Table \ref{tab:contam_table}).
