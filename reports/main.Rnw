...
Sequence data was simulated for \Sexpr{sum(single_tbl_df$N)} strains, from ten genera (Table \ref{tab:single_org}, Supplemental Table 1).
...
<<single_org_tree, fig.cap = "Dendrogram depicting the taxonomic lineage of genera used in baseline assessment.", echo=FALSE>>=
...
ref_genus_ids <- dnadiff_pair$ref_genus %>% 
      unique() %>% .[!is.na(.)]
ref_genus_labels <- ref_genus_ids %>% 
      {paste0(LETTERS[0:length(.)],": ",.)}

dnadiff_pair %>% filter(ref_genus == qry_genus) %>% 
      mutate(genus_lab = factor(ref_genus, 
                                levels = ref_genus_ids, 
                                labels = ref_genus_labels)) %>% 
      ggplot(aes(x = AlignedREF, y = AvgIdentity, color = ref_genus)) + 
      geom_density2d() + 
      facet_wrap(~genus_lab, nrow = 2) + 
      theme_bw() +
      labs(x = "Percent Aligned", 
...
<<contam_ani, echo = FALSE, message = FALSE, fig.cap = "Genomic similarity as percent of genome aligned and average nucleotide identity, between pairs of representative strains used in contaminant detection assessment.", fig.lp="fig:">>= 
ref_genus_ids <- contam_pairs$ref_genus %>% 
      unique() %>% .[!is.na(.)]
ref_genus_labels <- ref_genus_ids %>% 
      {paste0(LETTERS[0:length(.)],": ",.)}
contam_pairs %>% 
      ...
      facet_wrap(~genus_lab, nrow = 2) + scale_x_log10() + theme_bw() +
      theme(legend.position = "bottom") + 
...
contam_resid_summary <- contam_resid_summary %>%
      ungroup() %>% 
      mutate(target_facet = fct_reorder(target_facet, 
                                        prop_resid_total, median)) 

target_ids <- levels(contam_resid_summary$target_facet)
target_labels <- target_ids %>% {paste0(LETTERS[0:length(.)],": ",.)}

contam_resid_summary %>%
      mutate(target_labels = factor(target_facet, 
                                levels = target_ids, 
      ...
      ggplot() +
            ...
            facet_wrap(~target_labels, nrow = 1) + 
