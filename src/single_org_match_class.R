...
single_org_match <- singleOrgResults %>% 
...
single_org_match_taxid <- single_org_match %>% 
...
singleOrgMatch <- match_class_df %>% 
      rename(lca_name = name, lca_rank = rank, lca_id = id) %>% unique()

## Adding to project cache
cache("singleOrgMatch")

singleOrgMatchResults <- singleOrgMatch %>% left_join(single_org_match)

## Adding to project cache
cache("singleOrgMatchResults")
